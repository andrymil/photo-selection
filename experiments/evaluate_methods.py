...
def main():
    ...
    resnet_global = get_resnet18(
        DEVICE, weights_path=f"models/checkpoints/{EXPERIMENT_GLOBAL}.pth"
    )
    resnet_roi = get_resnet18(
        DEVICE, weights_path=f"models/checkpoints/{EXPERIMENT_ROI}.pth"
    )
    ...
    for path in tqdm(val_paths):
        ...
        results["pred_global_ai"].append(
            predict_ai(resnet_global, img_pil, transform, DEVICE)
        )
        ...
        results["pred_roi_ai"].append(
            predict_ai(resnet_roi, crop_pil, transform, DEVICE)
        )
    ...
    for name, preds in zip(methods_names, method_preds):
        a, p, r, f = print_metrics(name, val_labels, preds)
        acc_list.append(a)
        prec_list.append(p)
        rec_list.append(r)
        f1_list.append(f)
    ...
    rects2 = ax.bar(
        x - 0.5 * width, prec_list, width, label="Precision", color="#55A868"
    )
